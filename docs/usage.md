...
2. 顶部工具栏：
   ...
   - **文档切换**：仅 Mock 阶段可见（未装 dsh-structured-document 时切换
     会议纪要 / 项目管理 / 思路整理）；
   - **层级**：不限层级 / 1-5 层；
   - **布局**（思维导图）：逻辑图（右侧，默认）/ 思维导图（两侧）/ 上下；
   - **筛选**：按角色与属性（AND 组合，如「张三的进行中任务」）；
   - **恢复默认**：一键回默认视图（保留当前节点）；
3. Markdown 阅读器：从可搜索大纲或一级章节概览打开章节；“上一节/下一节”在同级章节间移动，“查看全文/只看本节”切换阅读范围；
4. 思维导图交互：默认显示两层并让节点在根节点右侧展开，适合窄侧边栏；选中节点若
   超出画布会自动以最小距离移入可视区；滚轮缩放、
   拖动画布平移、点击节点选中、按钮缩放/自适应/重置视口；Agent 下发聚焦时
   自动居中定位。单击显示内容预览，双击或点击“打开正文”进入对应章节；仅选中节点不会重置当前视口。
...
- 「找到当前节点」「显示全图」「放大一点」「改成从左到右布局」「恢复默认视图」
- 「只看待办事项」「只看进行中的任务」「清除筛选」

也可直接调用 17 个 View Tool（见 [tools.md](./tools.md)）。界面中的“你可以这样说”
提供常用短句，并可展开查看、搜索、复制或直接执行简单与组合指令。
