...
17 个工具，职责单一（**没有万能 command Tool**）。统一结果信封：
...
| `open_node` | `node?` | 在 Markdown 中打开节点及其全部子节点 | 打开第二技术路线 |
| `set_reader_mode` | `mode` | 切换当前章节或整篇文档 | 只看当前章节；查看全文 |
| `navigate_section` | `direction` | 在同一父节点下切换章节 | 上一节；下一节 |
